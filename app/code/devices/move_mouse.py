...
def drag_to_team_stats(bottom, left, right, top):
    ...
    pyautogui.dragTo(left + (right - left) // 2, top + 8 * (bottom - top) // 100, 1, button='left')
    time.sleep(2)
...
def drag_to_next_game(bottom, left, right, top):
    game_offset = 2 * (bottom - top) // 10
    ...
    time.sleep(2)
    # Slide to the top direction a 20% of the window height on the same x position
    pyautogui.dragTo(x_position, top + 54 * (bottom - top) // 100, 1, button='left')
    time.sleep(2)
